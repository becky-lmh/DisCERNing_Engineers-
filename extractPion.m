function pionCharge = extractPion(collision_tab)

% Remove particles that travel in negative z direction
collision_tab(collision_tab.p_z<0,:)=[];

% Extract pions
pi_plus = collision_tab(ismember(collision_tab.Secondary,'pi+'),:);
pi_minus = collision_tab(ismember(collision_tab.Secondary,'pi-'),:);

% Save pion data into a table
pionCharge = [pi_plus;pi_minus];

% Remove pions with mass-energy < 200 MeV to ensure they will travel to all
% 3 detectors
pionCharge(pionCharge.mass_energy_MeV<200,:)=[];

% Remove pions that don't have pseudorapidity 1.38 < eta < 3.88
px = table2array(pionCharge(:,"p_x"));
py = table2array(pionCharge(:,"p_y"));
pz = table2array(pionCharge(:,"p_z"));
eta = asinh(pz.*sin(atan(py./px))./py);
pionCharge = [pionCharge table(eta)];
pionCharge((-1.38<pionCharge.eta) & (pionCharge.eta<1.38),:)=[];
pionCharge(pionCharge.eta>3.88,:)=[];
pionCharge(pionCharge.eta<-3.88,:)=[];
